...
export default function SettingsPage() {
  ...
  return <section><div className="page-head"><div><span className="eyebrow">SETTINGS</span><h1>设置</h1><p>Nowen Forge {health?.version || 'V0.5'} · Token / Webhook Secret 只存在服务端环境变量</p></div></div>
    <div className="settings-grid">
      <div className="setting-card"><Github /><div><h3>GitHub Token</h3><p>一键发布需要 Fine-grained PAT 对 Nowen 仓库拥有 Contents: write + Actions: write；同时用于 Artifact 下载代理。</p>{health?.githubConfigured ? <span className="setting-ok"><CheckCircle2 size={15} />已配置</span> : <span className="setting-warn"><CircleAlert size={15} />未配置 GITHUB_TOKEN</span>}</div></div>
      ...
      <div className="setting-card"><Database /><div><h3>SQLite</h3><p>保存项目配置、Webhook 幂等记录、Manifest 与发布计划状态；不保存大体积制品文件。</p><span className="setting-ok"><CheckCircle2 size={15} />本地单用户模式</span></div></div>
